...
class Model(rankingmodel.Model):
    ...
    def forward(self, x, *, l2norm=False):
        ...
        if l2norm:
            ...
            raise ValueError("should not do l2norm on Euc embedding")
        return x
    ...
    def validate(self, dataloader):
        self.eval()
        allEmb, allLab = self.compute_embedding(dataloader,
                l2norm=False, device=self.device)
        correct = []
        for i in range(allEmb.shape[0]):
            pdist = (allEmb - allEmb[i].view(1,allEmb.shape[1])).norm(dim=1)
            knnsearch = allLab[pdist.topk(k=2, largest=False)[1][1]]
            correct.append((knnsearch == allLab[i]).item())
    ...
    def loss(self, x, y, *, marginE=1.0, hard=True):
        '''
        ...
        #dAP = th.nn.functional.pairwise_distance(output[0::3], output[1::3], p=2)
        #dAN = th.nn.functional.pairwise_distance(output[0::3], output[2::3], p=2)
        #loss = (dAP - dAN + marginE).clamp(min=0.).mean()
        loss = th.nn.functional.triplet_margin_loss(output[0::3],
                output[1::3], output[2::3], p=2, margin=marginE, reduction='mean')
