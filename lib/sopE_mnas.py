...
class Model(rankingmodel.Model):
    ...
    def __init__(self, finetune=True):
        ...
        if not finetune:
            cprint('Note, setting pretrain=False for MNasNet-1.0 as requested', 'red')
        mnas = vision.models.mnasnet1_0(pretrained=True if finetune else False)
        self.mnas= th.nn.DataParallel(mnas, dim=0)
        self.mnas.classifier = th.nn.Identity() # th.nn.Linear(2048, 2048)
        self.metric = 'E'
        self.cachepath = 'trained/__cache__.embs.sopE_mnas.th'
...
'''
Performance
Validate[41] r@mean 149.9 (/60502) r@1 0.653 (/1)r@10 0.811 (/1) r@100 0.915 (/1) r@1000 0.975 (/1)
'''
